...
object FunSets {
  ...
  /**
    ...
    */
  def singletonSet(elem: Int): Set = { otherElem: Int => otherElem == elem }
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      // if we itered all elements
      if (a > bound) true
      // if an element doesn't conform to function p
      else if (contains(s, a) && !p(a)) false
      //iter next
      else iter(a + 1)
    }
    //iter from initial val
    iter(-bound)
  }
  ...
  /**
    ...
    */
  def map(s: Set, f: Int => Int): Set = {elem:Int => exists(s, {otherElem:Int => f(otherElem) == elem})}
